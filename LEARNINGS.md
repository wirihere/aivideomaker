...
- ⚠ **`scripts/fetch-assets.mjs:59` still uses `shell: process.platform === "win32"`** — out of wave-F scope (not in `check`/`render`/`smoke` chain). Roll into the same `platform-bin.mjs` pattern next time `fetch-assets.mjs` is touched.
