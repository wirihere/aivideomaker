...
- **DEP0190 in `scripts/render.mjs` spawn** — Node 22 deprecation warning *"Passing args to a child process with shell option true can lead to security vulnerabilities"* fires every render. Found 2026-04-26 during wave-D progress-bar validation. Fix: pass argv as an array and drop `shell: true` (or use `cross-spawn` if shell-resolution of `npx` is required on Windows).
