...
- ⚠ **`scripts/usage.mjs` doesn't trace dynamic `fetch("path/to/X.json")` loads** — surfaced 2026-04-26 when `assets/amp/{kindred-bed,claim-mate-v2}.json` showed as 0-references but are read at runtime by `ampBind` via fetch. Other potentially-dynamic loads: any module that does `await fetch(somePath)` or `await import(somePath)`. Fix: extend `extractRefs()` in `scripts/usage.mjs` to also match `fetch\(["']([^"']+)["']\)` and `import\(["']([^"']+)["']\)` patterns inside `<script>` blocks and `.js` files. Until then, the unused list contains false positives — verify before deleting.
