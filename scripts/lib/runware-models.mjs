...
export const RECOMMENDED = {
  ...
  "image-gen": [
    { id: "runware:400@1", name: "FLUX.2 [dev]", pricePerCall: 0.016, capabilities: ["text-to-image", "image-to-image"], bestFor: "Cheap default for social content — carousels, stories, thumbnails ($0.016/image at 1088×1920 social size; $0.009 at 1024²). Serverless. Use for everything unless the detail visibly isn't there.", avoidFor: "Hero-quality shots where maximum prompt adherence and fine detail matter (use FLUX.2 [pro]).", verified: "2026-08-03" },
    { id: "bfl:5@1", name: "FLUX.2 [pro]", pricePerCall: 0.030, capabilities: ["text-to-image", "image-to-image"], bestFor: "Top-quality generation ($0.030/image at 1024²; scales with resolution). Better prompt adherence and fine detail than dev.", avoidFor: "Routine social content (dev is ~2× cheaper at social size and fine for carousels/stories).", verified: "2026-08-03" },
  ],
  tts: [
...
export const TRAPS = [
  ...
  { id: "mirelo:sfx@1.6", name: "Mirelo SFX 1.6", modality: "sfx", trap: "Listed in modelSearch as text-to-SFX (category: audio, caps: io:text-to-audio) but REJECTS audioInference with 'invalidModel: Please select a valid audio model.' Zero inference activity on the account — dark/unreachable. Do NOT trust the catalogue listing alone.", verified: "2026-08-03" },
  { id: "mirelo:1@1", name: "Mirelo SFX 1.5", modality: "sfx", trap: "This id DOES invoke via audioInference (flat params: duration, steps, positivePrompt) but it is AUDIO-TO-AUDIO only — does NOT follow text prompts. A test prompt for 'iOS message ding' returned an organic frog-croak unrelated to the prompt. $0.10/generation. Not usable for text-to-SFX. Fall back to the curated assets/sfx library or fetch recorded SFX from Pixabay (scripts/fetch-pixabay-sfx.mjs).", verified: "2026-08-03" },
];
